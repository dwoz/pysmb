...
def test_rename_english_file_SMB1(setup_func_SMB1, teardown_func):
    ...
    conn.storeFile(info['share'], old_path, BytesIO(b'Rename file test'))

    entries = conn.listPath(info['share'], os.path.dirname(old_path.replace('/', os.sep)))
    ...
    conn.rename(info['share'], old_path, new_path)
    ...
    conn.deleteFiles(info['share'], new_path)
...
def test_rename_english_directory_SMB1(setup_func_SMB1, teardown_func):
    ...
    conn.createDirectory(info['share'], old_path)
    ...
    conn.deleteDirectory(info['share'], new_path)
