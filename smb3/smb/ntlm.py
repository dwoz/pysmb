...
def expandDesKey(key):
    ...
    s = [ ((ord(key[0:1]) >> 1) & 0x7f) << 1,
          ((ord(key[0:1]) & 0x01) << 6 | ((ord(key[1:2]) >> 2) & 0x3f)) << 1,
          ((ord(key[1:2]) & 0x03) << 5 | ((ord(key[2:3]) >> 3) & 0x1f)) << 1,
          ((ord(key[2:3]) & 0x07) << 4 | ((ord(key[3:4]) >> 4) & 0x0f)) << 1,
          ((ord(key[3:4]) & 0x0f) << 3 | ((ord(key[4:5]) >> 5) & 0x07)) << 1,
          ((ord(key[4:5]) & 0x1f) << 2 | ((ord(key[5:6]) >> 6) & 0x03)) << 1,
          ((ord(key[5:6]) & 0x3f) << 1 | ((ord(key[6:7]) >> 7) & 0x01)) << 1,
          (ord(key[6:7]) & 0x7f) << 1
        ]
    return bytes(s)
    #else:
...
def generateChallengeResponseV1(password, server_challenge, has_extended_security = False, client_challenge = None):
    ...
    - [MS-NLMP]: 3.3.1
    """
    s_password = (password.upper() + '\0' * 14)[:14]
    _password = s_password.encode('ascii') #bytes(s_password, 'ascii')
